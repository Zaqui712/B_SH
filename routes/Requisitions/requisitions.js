  ...
  try {
    ...
    const query = `
      SELECT 
          ...
          E.descricao AS nomeEstadoRequisicao,   -- Corrected Estado table reference
      ...
      LEFT JOIN 
          SERVICOSDB.dbo.Estado E ON R.estadoID = E.estadoID   -- Estado table join with corrected column name
      LEFT JOIN 
...
router.put('/complete/:requisicaoID', verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    const requisitionDetailsQuery = `
      ...
      JOIN SERVICOSDB.dbo.Profissional_De_Saude ps ON r.profissionalID = ps.profissionalID

...
const verifyAdmin = async (req, res, next) => {
  ...
  try {
    ...
    const query = 'SELECT utilizadorAdministrador FROM dbo.Credenciais WHERE credenciaisID = @adminID';
...
router.post('/requests/create', async (req, res) => {
  ...
  try {
    const pool = await getPool();
    const query = `
      INSERT INTO dbo.Requisicao (estadoID, profissionalID, adminID, aprovadoPorAdministrador, requisicaoCompleta, dataRequisicao, dataEntrega)
...
router.get('/requests/', async (req, res) => {
  try {
    const pool = await getPool();
    const query = `SELECT * FROM dbo.Requisicao`; // Fetch all requests
...
router.get('/pending-approval', async (req, res) => {
  try {
    ...
    const query = `
      ...
      FROM dbo.Requisicao req
      JOIN dbo.Profissional_De_Saude pro ON req.profissionalID = pro.profissionalID
